...
public class Album {
	...
	public boolean excluirMusica(Long codMusica) {
		Musica aux = getMusicas().get(codMusica);
		
		if ( aux != null ) {
			getMusicas().remove(codMusica);
			return true;
	...
	public void setDatCriacao(LocalDate datCriacao) {
		try {
			if (datCriacao.isBefore(LocalDate.now().plusDays(1))) {
