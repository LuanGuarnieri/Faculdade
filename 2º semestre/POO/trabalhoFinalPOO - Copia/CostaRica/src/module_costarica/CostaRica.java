...
public class CostaRica  implements NationalTeamInfos {
	...
	public  static final String repositorio = "C:\\" + CostaRica.pais.replace(" ", ""); 
	...
	private void adicionaJsonJogadores() {
		...
		if (!dir.exists()) { 
			try {
				...
				insereJogadorJSON("PARIS-SAINT-GERMAIN", "KAYLOR NAVAS",          "NAVAS",       186, 75.0,  "11-06-1987", 1,  "GOLEIRO",          jsonArray);
				insereJogadorJSON("ROMA",            	 "DANIEL CORTEZ",         "CORTEZ",      173, 82.0,  "11-01-1992", 20, "LATERAL DIREITO",  jsonArray);
				insereJogadorJSON("MONTERREY",         	 "JOEL CAMPBELL",     	  "CAMPBELL",    178, 77.0,  "28-08-1989", 2,  "ZAGUEIRO",         jsonArray);
				insereJogadorJSON("SEVILLA",        	 "ERIC GONZALEZ",         "ERIC",        182, 76.0,  "09-01-2001", 12, "ZAGUEIRO",         jsonArray);
				insereJogadorJSON("ALVARADO",        	 "JORDI MARTINEZ", 		  "JORDI",       170, 68.0,  "21-03-1989", 33, "LATERAL ESQUERDO", jsonArray);
				insereJogadorJSON("WALTFORD",       	 "SERGIO SALAZAR",        "SALAZAR",     189, 76.0,  "16-06-1988", 5,  "VOLANTE",          jsonArray);
				insereJogadorJSON("ATLETIC BILBAO",   	 "JORGE  STROICO",        "KOKE",        176, 74.0,  "08-01-1992", 8,  "MEIA CENTRAL",     jsonArray);
				insereJogadorJSON("BARCELONA",       	 "PABLO GATZE",    		  "GATZE",       174, 60.0,  "25-11-2004", 9,  "MEIA DE ARMACAO",  jsonArray);
				insereJogadorJSON("LANUZ",	         	 "PEDRO RAVIER",          "RAVIER",      173, 62.0,  "11-01-2002", 26, "MEIA OFENSIVO",    jsonArray);
				insereJogadorJSON("LIVERPOOL",       	 "ANSSUMANE FATU VIERE",  "ANSU FATU",   178, 66.0,  "31-10-2002", 25, "PONTA DIREITA",    jsonArray);
				insereJogadorJSON("RENNES",         	 "ÁLVARO SANTOS",         "SANTOS",      189, 74.0,  "23-10-1992", 7,  "CENTRO AVANTE",    jsonArray);
				insereJogadorJSON("NEWCASTLE",       	 "NICHOLAS BERNADES",     "NICO",        181, 67.0,  "12-07-2002", 11, "PONTA ESQUERDA",   jsonArray);
	...
	private void adicionaJsonComissaoTecnica() {
		...
		if ( !dir.exists() ) {
			try {
				dir.mkdir();
				JSONArray jsonArray = new JSONArray();
				insereComissaoTecnicaJSON("BOCA JUNIORS", "LUIZ FERNANDO SUAREZ",  "SUAREZ", 180, 89.3, "08-05-1970", "TECNICO", jsonArray);
				...
				insereAcessorImprensaJSON("INDEFINIDO", "PALOMA ANTORANZ ESPINAR", "PALOMA", 165, 65.4, "14-10-1986", "22995010", "22995012", "paloma.espinar@ccr.cr", jsonArray);
	...
	protected void registrarLog(String msg) { // registro de logs
		try {
